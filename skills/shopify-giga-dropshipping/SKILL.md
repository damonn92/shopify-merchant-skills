...
1. Install GIGA app on each Shopify store you want to source for. Each install gets a unique `shopId` (e.g. the store = `1650`, the store = different).
...
```graphql
query {
 productsCount(query: "vendor:the store") { count }
}
```
...
4. **AI listing optimization** — Claude Haiku rewrites title/SEO/description/alt text. Idempotency marker `<!-- store-optimized-v1 -->` on `descriptionHtml` prevents re-running.
...
- Real outcome: 188 vendor:the store products, 175 ad-ready, 98.3% pass full Google/Meta audit
