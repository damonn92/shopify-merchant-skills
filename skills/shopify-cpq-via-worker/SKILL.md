...
Cart line title: "the store Custom Pricing" (rewritten via theme snippet)
```

### Worker code skeleton (TypeScript)

```typescript
// cpq-worker/src/index.ts
export default {
...
async function handleCpqDiscount(req: Request, env: Env) {
 ...
 const allowed = ['https://yourstore.com', 'https://<your-permanent-domain>.myshopify.com', /\.shopifypreview\.com$/];
...
async function addToCart(spec, variantId, productId) {
 ...
 const r = await fetch('https://cpq-worker.example.workers.dev/api/cpq-discount', {
 method: 'POST',
 headers: { 'Content-Type': 'application/json' },
...
{%- if discount_label contains "CPQ-" -%}
 {%- assign discount_label = "the store Custom Pricing" -%}
{%- endif -%}
...
- **Real product GIDs (the store, 8 live variants):** `<your-product-id>` (alcove-sliding) through `8582619168905` (corner-swing-x)
- **Metafield definition:** `cpq.config` JSON, pinned on Product, holds full pricing schema per variant
