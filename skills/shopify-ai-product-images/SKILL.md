...
Calivell result: deleted 574 white-bg images, generated 254 AI lifestyle scenes, **all 226 products now have ≥5 images**. Total cost: ~$5 (Vertex AI billing).
...
Calivell first-pass: **574 white-bg images deleted from 152 products, 0 failures, 30% image reduction.**
...
KZG: didn't need this (GIGA images are 2000×2000 white-bg with no watermark). Used `reorder_media.py` instead to put catalog shot at position 1.
...
- Idempotency: `kzg-optimized-v1` marker on description, image alt = `"{title} — lifestyle scene N"` for AI-generated
