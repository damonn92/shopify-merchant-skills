...
when_to_use:
 ...
 - The 3rd-party app intercepts the Checkout button and breaks something (Zapiet did this for the store)
not_for:
...
<style>
 .cart-delivery-form { padding: 16px; border: 1px solid #e5dfd3; border-radius: 8px; margin-bottom: 16px; }
 .cart-delivery-form label { display: block; margin-bottom: 12px; font-size: 14px; }
 .cart-delivery-form input, .cart-delivery-form select, .cart-delivery-form textarea {
 display: block; width: 100%; margin-top: 4px; padding: 8px; border: 1px solid #e5dfd3; border-radius: 4px;
 }
 .cart-fee-summary { padding: 8px; background: rgba(204,120,92,0.08); border-radius: 4px; }
</style>
```
...
```javascript
document.querySelectorAll('.cart-delivery-form select, .cart-delivery-form input').forEach(el => {
...
- the store fee schedule (real-world numbers):
 - Pickup: $0
 - Delivery only: $150
