...
1. **The CLI rejects the custom domain.** `shopify store auth --store <your-custom-domain>` will fail because Shopify's OAuth callback only accepts the **permanent** `<your-permanent-domain>.myshopify.com` form. The custom domain is for shoppers, not admins.
...
- **The CLI does not lock state across processes.** If two scripts run `shopify store execute` simultaneously, one may get an auth error. Stagger any cron-driven scripts on different minutes (stagger to 9:15 to avoid simultaneous CLI auth).
