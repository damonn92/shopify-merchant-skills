...
function showStep(step) {
 ...
 el.classList.toggle('dc-step--active', el.dataset.dcStep === step);
 });
 STATE.step = step;
 saveToURL();
}
...
→ /products/<your-product>?from=design-center&... Step 6 (PDP, CPQ configurator)
```
...
function recomputePrice() {
 ...
 document.querySelector('.dc-running-total').textContent = `$${p}`;
}
```
...
- the store Design Center live: `https://yourstore.com/pages/design-center`
